...
llm_graph_result * llama_context::process_ubatch(
    ...
    // //slot-layers map
    // {
    //     auto * slot_layers_map_tensor = ggml_graph_get_tensor(gf, "layer_to_slot");
    //     if (slot_layers_map_tensor) {
    //         ggml_backend_tensor_set(slot_layers_map_tensor, layer_to_slot_data.data(), 0, layer_to_slot_data.size() * sizeof(int32_t));
    //     }
    // }
...
llm_graph_result * llama_context::process_ubatch_with_slot(
    const llama_ubatch & ubatch, 
    llm_graph_type gtype, 
    llama_memory_context_i * mctx, 
    ggml_status & ret,
    ...
    //slot-layers map
    {
        auto * slot_layers_map_tensor = ggml_graph_get_tensor(gf, "layer_to_slot");
        if (slot_layers_map_tensor) {
            ggml_backend_tensor_set(slot_layers_map_tensor, layer_to_slot_data, 0, model.hparams.n_layer * sizeof(int32_t));
        }
    }
...
int llama_context::encode_with_slot(const llama_batch & batch_inp, const int32_t * layer_to_slot_data) {
    GGML_ASSERT((!batch_inp.token && batch_inp.embd) || (batch_inp.token && !batch_inp.embd)); // NOLINT
    ...
    ggml_status status;
    const auto * res = process_ubatch_with_slot(ubatch, LLM_GRAPH_TYPE_ENCODER, nullptr, status, layer_to_slot_data);
...
int llama_context::decode_with_slot(const llama_batch & batch_inp, const int32_t * layer_to_slot_data) {
    GGML_ASSERT((!batch_inp.token && batch_inp.embd) || (batch_inp.token && !batch_inp.embd)); // NOLINT

    if (!memory) {
        LLAMA_LOG_DEBUG("%s: cannot decode batches with this context (calling encode() instead)\n", __func__);
        return encode_with_slot(batch_inp, layer_to_slot_data);
    }
    ...
    do {
        ...
        ggml_status status;
        const auto * res = process_ubatch_with_slot(ubatch, LLM_GRAPH_TYPE_DECODER, mctx.get(), status, layer_to_slot_data);
...
int32_t llama_decode_with_slot(
        llama_context * ctx,
        llama_batch    batch,
        const int32_t * layer_to_slot_data) {

    const int ret = ctx->decode_with_slot(batch, layer_to_slot_data);
    if (ret != 0 && ret != 1) {
        LLAMA_LOG_ERROR("%s: failed to decode, ret = %d\n", __func__, ret);
    }
...
int32_t llama_encode_with_slot(
        llama_context * ctx,
        llama_batch   batch,
        const int32_t * layer_to_slot_data) {

    const int ret = ctx->encode_with_slot(batch, layer_to_slot_data);
    if (ret != 0) {
        LLAMA_LOG_ERROR("%s: failed to encode, ret = %d\n", __func__, ret);
    }
